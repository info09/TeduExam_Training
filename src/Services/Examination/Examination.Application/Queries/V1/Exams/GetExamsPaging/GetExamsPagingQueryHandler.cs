...
namespace Examination.Application.Queries.V1.Exams.GetExamsPaging
{
    public class GetExamsPagingQueryHandler : IRequestHandler<GetExamsPagingQuery, ApiResult<PagedList<ExamDto>>>
    {
        ...
        {
            var result = await _examRepository.GetExamsPagingAsync(request.CategoryId, request.SearchKeyword,
                request.PageIndex, request.PageSize);

            var items = _mapper.Map<List<ExamDto>>(result.Items);

            return new ApiSuccessResult<PagedList<ExamDto>>((int)HttpStatusCode.OK,
                new PagedList<ExamDto>(items, result.MetaData.TotalCount, request.PageIndex, request.PageSize));

...
namespace Identity.STS.Identity.Services
{
    public class CustomeProfileService : IProfileService
    {
        ...
        {
            _logger.LogDebug("Get profile called for subject {subject} from client {client} with claim types {claimTypes} via {valler}", context.Subject.GetSubjectId(), context.Client.ClientName ?? context.Client.ClientId, context.RequestedClaimTypes, context.Caller);

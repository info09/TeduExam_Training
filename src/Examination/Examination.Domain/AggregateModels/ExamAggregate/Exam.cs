...
namespace Examination.Domain.AggregateModels.ExamAggregate
{
    ...
    {
        public Exam(string name, string shortDesc, string content, int numberOfQuestions, TimeSpan duration, IEnumerable<Question> questions, Level level, string ownerUserId, int numberOfQuestionCorrectForPass, bool isTimeRestricted)
        {
            if (questions == null && !questions.Any())
                throw new ArgumentNullException($"{nameof(questions)} can not be null");

            if (questions.Count() != numberOfQuestions)
                throw new ArgumentNullException($"{nameof(numberOfQuestions)} is valid");

            if (numberOfQuestionCorrectForPass > numberOfQuestions)
                throw new ArgumentNullException($"{nameof(numberOfQuestionCorrectForPass)} is valid");
